...
//     if (!expDateStr) return { status: 'valid', text: '', color: '' };
...
//     if (diffDays < 0) {
//         return { status: 'expired', text: 'Đã hết hạn', color: 'bg-red-50 dark:bg-red-900/10 border-red-200' };
//     } else if (diffDays <= 30) {
//         return { status: 'warning', text: `Cận date (${diffDays} ngày)`, color: 'bg-orange-50 dark:bg-orange-900/10 border-orange-200' };
//     }
//     return { status: 'valid', text: 'Còn hạn', color: 'border-gray-100 dark:border-white/5 hover:bg-gray-50' };
// };
...
import Pagination from '../../components/Pagination';
import CurrencyInput from '@/components/CurrencyInput';
import { useConfirmDialog } from '../../components/ConfirmDialog'; // <-- Thêm import này
...
const InventoryManager: React.FC = () => {
    ...
    const { confirm } = useConfirmDialog(); // <-- Hook popup xác nhận
    ...
    // --- LOGIC XUẤT HỦY ---
    const handleDisposeBatch = async (batchId: number, quantityToDispose: number) => {
        const ok = await confirm({
            title: 'Xuất hủy lô hàng',
            message: `Lô hàng này đã hết hạn. Bạn có chắc muốn xuất hủy ${quantityToDispose} sản phẩm còn tồn? Số lượng này sẽ được trừ khỏi tổng tồn kho của sản phẩm.`,
        ...
        try {
            await adminInventoryApi.disposeBatch(batchId);
            setMsg({ type: 'success', text: 'Đã xuất hủy lô hàng hết hạn thành công!' });
            fetchBatches(); 
            fetchProducts(); 
        } catch (err: any) {
            setMsg({ type: 'error', text: err?.message || 'Lỗi khi xuất hủy' });
        } finally {
            setTimeout(() => setMsg(null), 3000);
        }
    };
    ...
    const getExpiryStatus = (expDateStr: string) => {
        ...
        if (diffDays < 0) {
            return { status: 'expired', text: 'Đã hết hạn', color: 'bg-red-50/80 dark:bg-red-900/10 border-red-200 dark:border-red-900/30' };
        } else if (diffDays <= 30) {
            return { status: 'warning', text: `Cận date (${diffDays} ngày)`, color: 'bg-orange-50/80 dark:bg-orange-900/10 border-orange-200 dark:border-orange-900/30' };
        }
        return { status: 'valid', text: 'Còn hạn', color: 'border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/[0.02]' };
    };
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Modal Nhập kho */}
            {showForm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 z-[100]">
                    <div className="bg-white dark:bg-card-dark rounded-2xl p-6 w-full max-w-lg shadow-2xl border border-gray-200 dark:border-white/10">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Nhập lô hàng mới</h3>
                        <div className="space-y-4">
                            <div>
            ...
            <div className="bg-white dark:bg-card-dark rounded-2xl border border-gray-200 dark:border-white/5 overflow-hidden shadow-sm transition-all">
                <div className="overflow-x-auto">
                    <table className="w-full">
                        ...
                        <tr className="border-b border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-surface-darker text-left">
                            <th className="px-5 py-3 text-xs font-bold text-gray-500 uppercase w-[15%]">Mã Lô</th>
                            <th className="px-5 py-3 text-xs font-bold text-gray-500 uppercase w-[25%]">Sản phẩm</th>
                            <th className="px-5 py-3 text-xs font-bold text-gray-500 uppercase text-center w-[15%]">Tồn kho / Nhập</th>
                            <th className="px-5 py-3 text-xs font-bold text-gray-500 uppercase text-right w-[15%]">Giá nhập</th>
                            <th className="px-5 py-3 text-xs font-bold text-gray-500 uppercase text-center w-[15%]">Hạn sử dụng</th>
                            <th className="px-5 py-3 text-xs font-bold text-gray-500 uppercase text-right w-[15%]">Thao tác</th>
                        </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                        {loading ? (
                            <tr><td colSpan={6} className="text-center py-12"><span className="material-symbols-outlined animate-spin text-3xl text-primary">progress_activity</span></td></tr>
                        ) : batches.length === 0 ? (
                            <tr>
                            ...
                            const stockPercent = (batch.currentQuantity / batch.importQuantity) * 100;
                            // Tính toán ngày hết hạn để tô màu
                            const expiryInfo = getExpiryStatus(batch.expiryDate);

                            return (
                                <tr key={batch.id} className={`border-b transition-colors ${expiryInfo.color}`}>
                                    ...
                                    <td className="px-5 py-4 text-center">
                                        <p className={`text-sm font-medium ${expiryInfo.status === 'expired' ? 'text-red-600 line-through' : 'text-gray-900 dark:text-white'}`}>
                                            {batch.expiryDate ? new Date(batch.expiryDate).toLocaleDateString('vi-VN') : '—'}
                                        </p>
                                        <p className="text-[10px] text-gray-400 mb-1">
                                            NSX: {batch.manufactureDate ? new Date(batch.manufactureDate).toLocaleDateString('vi-VN') : '—'}
                                        </p>
                                        {/* Badge hiển thị trạng thái hạn sử dụng */}
                                        {expiryInfo.status !== 'valid' && (
                                            <span className={`px-2 py-0.5 text-[10px] font-bold rounded-md ${
                                                expiryInfo.status === 'expired' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'
                                            }`}>
                                                {expiryInfo.text}
                                            </span>
                                        )}
                                    ...
                                    <td className="px-5 py-4 text-right">
                                        {/* Chỉ hiện nút xuất hủy nếu lô hàng đã hết hạn và còn tồn kho */}
                                        {expiryInfo.status === 'expired' && batch.currentQuantity > 0 && (
                                            <button 
                                                title="Xuất hủy lô hàng hết hạn"
                                                className="flex items-center gap-1 px-3 py-1.5 bg-red-100 text-red-600 hover:bg-red-200 rounded-lg text-xs font-bold ml-auto shadow-sm transition-colors"
                                                onClick={() => handleDisposeBatch(batch.id, batch.currentQuantity)}
                                            >
