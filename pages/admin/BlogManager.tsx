...
const PostSection: React.FC<{ setMsg: (m: { type: 'success' | 'error'; text: string } | null) => void }> = ({ setMsg }) => {
  ...
  const handleSubmit = async () => {
    if (!form.title.trim()) { setMsg({ type: 'error', text: 'Tiêu đề không được trống' }); setTimeout(() => setMsg(null), 3000); return; }
    // Thay đổi kiểm tra content (TinyMCE có thể trả về '<p><br></p>')
    if (!form.content.trim() || form.content === '<p><br data-mce-bogus="1"></p>') { 
      setMsg({ type: 'error', text: 'Nội dung không được trống' }); 
      setTimeout(() => setMsg(null), 3000); 
      return; 
    }
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={resetForm}>
          {/* Đổi kích thước max-w-2xl thành max-w-4xl để Editor hiển thị đẹp hơn */}
          <div className="bg-white dark:bg-card-dark rounded-2xl p-6 w-full max-w-4xl shadow-2xl border border-gray-200 dark:border-white/10 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              {/* --- TINYMCE: Bắt đầu thay thế Textarea --- */}
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nội dung *</label>
                <Editor
